...
public class Servicio
{
  ...
  public Response consulta(@FormParam("id_usuario") Integer id_usuario) throws Exception
  {
    ...
    try
    {
      PreparedStatement stmt_1 = conexion.prepareStatement("SELECT a.email,a.nombre,a.apellido_paterno,a.apellido_materno,a.fecha_nacimiento,a.telefono,a.genero,b.foto FROM usuarios a LEFT OUTER JOIN fotos_usuarios b ON a.id_usuario=b.id_usuario WHERE id_usuario=?");
      try
      {
  ...
  public Response borra(@FormParam("id_usuario") Integer id_usuario) throws Exception
  {
    ...
    try
    {
      ...
      try
      {
        ...
        {
          if (!rs.next())
		return Response.status(400).entity(j.toJson(new Error("El usuario no existe"))).build();
        }
        finally
        {
          rs.close();
